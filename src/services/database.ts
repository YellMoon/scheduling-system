...
class DatabaseService {
  ...
  constructor() {
    // 优先使用 Electron userData 路径（重装不丢失）
    // 渲染进程中通过 IPC 获取，如果不可用则回退
    ...
    try {
      ...
      // IPC 不可用，回退到旧逻辑
      userDataPath = process.env.NODE_ENV === 'production' 
        ? process.resourcesPath 
        : __dirname;
    }
    // 确保 userData 目录下的子目录存在
  ...
  createCourse(course: Omit<Course, 'id' | 'created_at' | 'updated_at'>): Course {
    const id = uuidv4();
    this.run(`
      INSERT INTO courses (id, name, type, source_type, price_tuition, price_teacher, billing_unit, room_name, teacher_id, teacher_name, notes, year, semester, display_name, active, price_teacher_mode, student_pricings, room_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [id, course.name, course.type, course.source_type, course.price_tuition, course.price_teacher, course.billing_unit,
        course.room_name, course.teacher_id, course.teacher_name, course.notes, course.year, course.semester, course.display_name, 
        course.active, course.teacher_fee_mode, JSON.stringify(course.student_pricings), course.room_id
      ]);
