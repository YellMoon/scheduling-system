...
const COOKIE_FILE = path.join(process.env.LOCALAPPDATA || process.env.TEMP || '.', 'qoder-quark-cookies.json');
const PROFILE_DIR = path.join(process.env.LOCALAPPDATA || process.env.TEMP || '.', 'qoder-quark-profile');
const today = new Date();
const DATE_FOLDER = `${today.getFullYear()}-${String(today.getMonth()+1).padStart(2,'0')}-${String(today.getDate()).padStart(2,'0')}`;
const ROOT_FOLDER = 'Qoder项目';
const ROOT_FOLDER_ALIASES = ['Qoder项目', 'qoder项目', 'Qoder項目'];
...
(async () => {
  ...
  try {
    ...
    const shot = path.join(process.env.TEMP || '.', 'qoder-quark-upload.png');
