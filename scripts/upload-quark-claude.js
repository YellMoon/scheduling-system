...
const COOKIE_FILE = path.join(process.env.LOCALAPPDATA || process.env.TEMP || '.', 'claude-quark-cookies.json');
const PROFILE_DIR = path.join(process.env.LOCALAPPDATA || process.env.TEMP || '.', 'claude-quark-profile');
const today = new Date();
const DATE_FOLDER = `${today.getFullYear()}-${String(today.getMonth()+1).padStart(2,'0')}-${String(today.getDate()).padStart(2,'0')}`;
const ROOT_FOLDER = 'Claude项目';
...
async function dblClickByName(page, name) {
  const pos = await page.evaluate((n) => {
    const selectors = [
      '[class*="filename-text"]', '[class*="file-name"]', '[class*="name"]',
      '[data-node-name]', 'span[title]', '[class*="item-name"]',
    ];
    for (const sel of selectors) {
...
async function ensureFolder(page, name) {
  ...
  if (!names.includes(name)) {
    ...
    // Reload to refresh file list after folder creation
    await page.reload({ waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(3000);
    names = await getItemNames(page);
    if (!names.includes(name)) {
      throw new Error('Failed to create folder: ' + name + '. Found: ' + names.join(', '));
    }
  }
  // enter the folder
...
(async () => {
  ...
  try {
    ...
    // Wait extra for page to fully render file list
