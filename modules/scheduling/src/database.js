...
class DatabaseService {
  ...
  _init() {
    ...
    // 迁移: students 表添加 user_id 列
    try {
      this.db.prepare('SELECT user_id FROM students LIMIT 1').get();
    } catch {
      this.db.prepare('ALTER TABLE students ADD COLUMN user_id TEXT').run();
      console.log('[DB] 迁移: students 表添加 user_id 列');
    }

    // 迁移: teachers 表添加 user_id 列
    try {
      this.db.prepare('SELECT user_id FROM teachers LIMIT 1').get();
    } catch {
      this.db.prepare('ALTER TABLE teachers ADD COLUMN user_id TEXT').run();
      console.log('[DB] 迁移: teachers 表添加 user_id 列');
    }
  ...
  getStudentByUserId(userId) {
    return this.db.prepare('SELECT * FROM students WHERE user_id = ? AND deleted = 0').get(userId);
  }

  getTeacherByUserId(userId) {
    return this.db.prepare('SELECT * FROM teachers WHERE user_id = ? AND deleted = 0').get(userId);
  }

  getCoursesByTeacherId(teacherId) {
    return this.db.prepare('SELECT * FROM courses WHERE teacher_id = ? AND deleted = 0').all(teacherId);
  }
